...
def comprar_produto() -> None:
    if len(produtos) > 0:
        print('Informe o códgio do produto que deseja adicionar  ao carrinho: ')
        print('---------------------------------------------------------------')
...
def fechar_pedido() -> None:
    ...
    if len(carrinho) > 0:
        ...
        for item in carrinho:
            for dados in item.items():
                print(dados[0])
                print(f'Quanatidade: {dados[1]}')
                valor_total += dados[0].preco * dados[1]
                print("-----------------------------")
                sleep(1)
        print(f'O valor da fatura é {formata_float_str_moeda(valor_total)}')
        print('Volte sempre!')
        carrinho.clear()
        sleep(5)

    else:
        print('Ainda não exite produto no carrinho')
    sleep(2)
    menu()
